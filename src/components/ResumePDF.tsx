    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ textAlign: 'center', marginBottom: 10 }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold', textTransform: 'uppercase' }}>{resumeData.personalInfo.name}</Text>
          <Text style={{ fontSize: 14, fontWeight: 'bold', marginBottom: 5 }}>FULL STACK DEVELOPER</Text>
        ...
        {resumeData.summary && (
          <View style={styles.section}>
            ...
            <View style={{ borderBottom: 1, borderBottomColor: '#888', marginBottom: 5 }} />
            ...
            {Object.entries(groupedSkills).map(([category, skillsList]) => (
              skillsList.length > 0 ? (
                <View key={category} style={{ marginBottom: 5 }}>
                  <Text>
                    <Text style={{ fontWeight: 'bold' }}>{category}:</Text> {skillsList.join(', ')}
                  </Text>
                </View>
              ) : null
            ))}
          ...
          <View style={styles.section}>
            ...
            {resumeData.workExperience.map((exp) => (
              <View key={exp.id} style={{ marginBottom: 10 }}>
                ...
                {exp.description && <Text style={{ marginBottom: 3 }}>{exp.description}</Text>}
        ...
        {resumeData.certifications?.length > 0 && (
          <View style={styles.section}>
            ...
            {resumeData.certifications.map((cert) => (
              <View key={cert.id} style={{ flexDirection: 'row', marginBottom: 3, paddingLeft: 10 }}>
                <Text style={{ width: 10 }}>•</Text>
                <Text style={{ flex: 1 }}>
        ...
        {resumeData.languages?.length > 0 && (
          <View style={styles.section}>
            ...
            {resumeData.languages.map((lang) => (
              <View key={lang.id} style={{ flexDirection: 'row', marginBottom: 3, paddingLeft: 10 }}>
                <Text style={{ width: 10 }}>•</Text>
                <Text style={{ flex: 1 }}>
